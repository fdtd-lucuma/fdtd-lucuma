...
public:
	...
	)
	{
		...
		#pragma omp parallel for default(private) firstprivate(x,y,z,Cr,imp0,deltaT) collapse(3) simd
	...
	)
	{
		...
		#pragma omp parallel for default(private) firstprivate(x,y,z) collapse(3) simd
		for(std::size_t i = 0; i < x; i++)
		{
			for(std::size_t j = 0; j < y; j++)
			{
